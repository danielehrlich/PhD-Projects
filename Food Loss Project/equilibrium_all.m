function [c, p, iter, error] = equilibrium_all(params)
    ...
    Vl = u(M);
    Vm_array = u(p_array.*x);
    norm_const = 1 - betacdf(p_array./pa, om, phi);
    incomplete_beta_norm = 1 - betacdf(p_array./pa, om + 1, phi);
    ex_value_array = (om / (om+ phi)) .* incomplete_beta_norm ./ norm_const; % calculate expected profit conditional on threshold E(X | X > x) 
    Jm_array = Omega(p_array./pa).*(pa.*ex_value_array- p_array)*x;
    theta_array = (Jm_array./kappa).^(1/alpha);
    ftheta_array = theta_array.^(1-alpha);
    while ((iter < 300) && (error > 1e-15))
        Vhat = Vm_array - Vs;
        Vhat(Vhat < 0) = 0;
        [~, pidx]  = max(ftheta_array.*Omega(p_array./pa).*Vhat);
        p = p_array(pidx);
